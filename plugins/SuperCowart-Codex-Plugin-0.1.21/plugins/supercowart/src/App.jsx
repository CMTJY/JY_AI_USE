...
import {
  ...
  downloadSuperCowartFile,
  hasSuperCowartWidgetBridge,
  loadSuperCowartCanvasState,
  readSuperCowartPageAsset,
  refreshSuperCowartCanvasSnapshot,
  saveSuperCowartCanvasSnapshot,
  saveSuperCowartReferenceImage,
  saveSuperCowartSelectionState,
  saveSuperCowartViewState,
  updateSuperCowartHtmlDraft
...
const ANNOTATION_EDIT_PROMPT = [
  '[@supercowart](plugin://supercowart@personal) 按标注修改',
  '',
  '请根据这张 SuperCowart 截图里的标注修改当前选中的图片：',
...
const AI_HTML_LOCAL_ASSET_PROMPT_LINES = [
  '- HTML 中不要引用 http:// 或 https:// 远程图片；SuperCowart widget 的 CSP 不允许 HTML iframe 直接加载这些资源。',
  ...
  '- SuperCowart 会在将 HTML 放入 iframe 前，通过 read_cowart_page_asset 把 /page-assets/ 图片转换为 data: URL。'
]
const ANNOTATION_HTML_PROMPT = [
  '[@supercowart](plugin://supercowart@personal) 按标注生成 AI HTML',
  '',
  '请根据这张 SuperCowart 截图里的当前图片和周围标注，生成一个新的单文件 HTML 草稿：',
...
const HTML_DRAFT_ANNOTATION_EDIT_PROMPT = [
  '[@supercowart](plugin://supercowart@personal) 按标注修改 AI HTML',
  '',
  '请根据这张 SuperCowart 截图里的标注修改当前选中的 HTML 草稿：',
...
const HTML_DRAFT_ANNOTATION_IMAGE_PROMPT = [
  '[@supercowart](plugin://supercowart@personal) 按标注生图',
  '',
  '请使用内置 imagegen skill，根据这张 SuperCowart 截图里的 HTML 草稿和标注生成一张新的干净位图：',
  '- 截图包含当前 HTML 草稿，以及草稿周围的标注箭头和标注文字。',
...
const AI_IMAGE_GENERATION_PROMPT_PREFIX = [
  '[@supercowart](plugin://supercowart@personal) 生成图片',
  '',
  '请根据下面的 prompt 生成一张图片，并替换当前选中的 SuperCowart AI 图片框；最终画布里应留下普通图片形状，不保留 AI 图片框容器。',
...
const AI_DRAFT_GENERATION_PROMPT_PREFIX = [
  '[@supercowart](plugin://supercowart@personal) 生成 AI HTML',
  '',
  '请根据下面的 prompt 生成一个单文件 HTML 草稿，并把它嵌入当前选中的 SuperCowart AI HTML 框。',
  ...
  '请生成完整可运行的 HTML 文档，CSS 和 JS 尽量内联，适合直接放进 iframe 预览。',
  ...AI_HTML_LOCAL_ASSET_PROMPT_LINES,
  '完成后调用 SuperCowart MCP 工具 insert_cowart_html_draft，把 htmlContent 写入当前 page 的 canvas/pages/<page-id>/assets/，并替换对应 AI HTML 框为 HTML embed。'
].join('\n')
const AI_SLIDES_GENERATION_PROMPT_PREFIX = [
  '[@supercowart](plugin://supercowart@personal) 生成 AI Slides',
  '',
...
const AI_SLIDES_ANNOTATION_EDIT_PROMPT = [
  '[@supercowart](plugin://supercowart@personal) 按标注修改 AI Slides',
  '',
  '请根据 SuperCowart 截图中的原 AI Slides 和周围标注，生成一套修改后的新 Slides。',
...
const cowartAssetUrls = buildSuperCowartAssetUrls()
...
const cowartTldrawAssetStore = {
  ...
  resolve: resolveSuperCowartTldrawAssetUrl
}

if (typeof window !== 'undefined') {
  window.addEventListener('beforeunload', revokeSuperCowartAssetObjectUrls, { once: true })
}

function buildSuperCowartAssetUrls() {
  const icons = {}
  ...
  try {
    ...
    console.warn('SuperCowart could not load bundled tldraw asset URLs.', error)
  }
  return { ...base, icons: { ...base.icons, ...icons } }
}

function isSuperCowartLocalAssetUrl(src) {
...
async function hydrateSuperCowartHtmlDraftLocalImages(htmlContent) {
  if (typeof htmlContent !== 'string' || !htmlContent || !hasSuperCowartWidgetBridge()) return htmlContent
  ...
  const replacements = await Promise.all(
    Array.from(references.entries()).map(async ([reference, assetUrl]) => {
      try {
        const asset = await readSuperCowartPageAsset(assetUrl)
        ...
        console.warn(`SuperCowart could not hydrate HTML draft image ${assetUrl}.`, error)
        return null
      }
    })
  )
...
function cowartHtmlDraftAssetUrlFromVirtualUrl(src) {
  ...
  try {
    ...
    return isSuperCowartHtmlDraftAssetUrl(assetUrl) ? assetUrl : null
  } catch (_error) {
    return null
  }
}
...
function revokeSuperCowartCachedAsset(cacheKey) {
...
function revokeSuperCowartAssetObjectUrls() {
  for (const cacheKey of Array.from(cowartAssetObjectUrlCache.keys())) {
    revokeSuperCowartCachedAsset(cacheKey)
  }
}
...
async function resolveSuperCowartTldrawAssetUrl(asset) {
  const src = asset?.props?.src
  if (!src) return null
  if (!hasSuperCowartWidgetBridge() || !isSuperCowartLocalAssetUrl(src)) return src
  ...
  if (previousKey && previousKey !== cacheKey) {
    revokeSuperCowartCachedAsset(previousKey)
  }

  try {
    const pageAsset = await readSuperCowartPageAsset(src)
    ...
    console.warn('SuperCowart could not resolve local page asset through MCP; falling back to source URL.', error)
    return src
  }
}
...
function isSuperCowartAiHolderShape(shape) {
  return isAiImageHolderShape(shape) || isAiDraftHolderShape(shape)
}

function isAiSlidesItemShape(shape) {
  return isImageShape(shape) || isSuperCowartHtmlDraftEmbedShape(shape)
}

function isSuperCowartHtmlDraftEmbedShape(shape) {
  ...
  return Boolean(
    cowartHtmlDraftAssetUrlFromVirtualUrl(shape.props?.url) ||
      isSuperCowartHtmlDraftDataUrl(shape.props?.url)
  )
}
...
function isAiImageAspectLocked(shape) {
  return isSuperCowartAiHolderShape(shape) && shape.meta?.cowartAiAspectLocked === true
}
...
function copySelectedSuperCowartContent(editor, event) {
...
function collectHtmlDraftAnnotationShapeIds(editor, draftShapeId) {
  return collectAnnotationTargetShapeIds(
    editor,
    draftShapeId,
    isSuperCowartHtmlDraftEmbedShape,
...
function buildAnnotationEditPrompt({ imageShapeId, shapeIds, exportWidth, exportHeight, screenshotAsset }) {
  ...
  const lines = [
    ...
    '',
    `SuperCowart source image shape: ${imageShapeId}`,
...
function buildAnnotationHtmlPrompt({ imageShape, shapeIds, exportWidth, exportHeight, screenshotAsset }) {
  ...
  return [
    ...
    '',
    `SuperCowart source image shape: ${imageShape.id}`,
...
function supportsSuperCowartMessageImages() {
...
async function sendAnnotationEditRequest(editor, imageShapeId) {
  ...
  const screenshotAsset = await saveSuperCowartReferenceImage({
  ...
  if (!sender) {
    throw new Error('当前 SuperCowart 画布没有可用的 Codex MCP 消息桥。')
  }

  const content = [{ type: 'text', text: prompt }]

  if (supportsSuperCowartMessageImages()) {
    content.push(
...
async function sendAnnotationHtmlRequest(editor, imageShapeId) {
  ...
  const sender = followUpSender()
  if (!sender) throw new Error('当前 SuperCowart 画布没有可用的 Codex MCP 消息桥。')
...
function serializeSuperCowartHtmlDraftDocument(iframeDocument) {
...
function isSuperCowartDomTextElement(element) {
...
function placeSuperCowartDomCaret(iframeDocument, element, clientX, clientY) {
...
function captureSuperCowartDomTextSelection(iframeDocument) {
  const selection = iframeDocument.getSelection()
...
function restoreSuperCowartDomTextSelection(iframeDocument, selectionSnapshot) {
...
function createSuperCowartHtmlDraftDomEditorSession(iframeDocument, { onSave, onRequestExit }) {
  ...
  function selectElement(element, event) {
    const textSelection = captureSuperCowartDomTextSelection(iframeDocument)
    ...
    if (!isSuperCowartDomTextElement(selectedElement)) return
    ...
    if (!restoreSuperCowartDomTextSelection(iframeDocument, textSelection)) {
      placeSuperCowartDomCaret(iframeDocument, selectedElement, event.clientX, event.clientY)
    }
  }
  ...
  async function flush() {
    ...
    const htmlContent = serializeSuperCowartHtmlDraftDocument(iframeDocument)
...
function getSuperCowartHtmlDraftAssetUrl(shape) {
  return (
...
function getSuperCowartHtmlDraftLocalPath(shape) {
  const projectDir = window.openai?.toolOutput?.projectDir
  const assetUrl = getSuperCowartHtmlDraftAssetUrl(shape)
...
async function renderSuperCowartHtmlDraftCanvas(shape, pixelRatio) {
  if (!isSuperCowartHtmlDraftEmbedShape(shape)) {
...
async function readSuperCowartHtmlDraftContent(shape) {
  if (!isSuperCowartHtmlDraftEmbedShape(shape)) {
    throw new Error('请选择一个已生成 HTML 的 AI HTML。')
  }

  const directHtmlUrl = isSuperCowartHtmlDraftDataUrl(shape.props?.url) ? shape.props.url : null
  if (directHtmlUrl) {
  ...
  if (hasSuperCowartWidgetBridge()) {
    const pageAsset = await readSuperCowartPageAsset(assetUrl)
...
async function readSuperCowartImageDataUrl(editor, imageShape) {
  ...
  if (sourceUrl.startsWith(PAGE_ASSETS_ROUTE) && hasSuperCowartWidgetBridge()) {
    const pageAsset = await readSuperCowartPageAsset(sourceUrl)
...
async function downloadSuperCowartImageShape(editor, imageShape) {
  const asset = imageShape?.props?.assetId ? editor.getAsset(imageShape.props.assetId) : null
  const sourceUrl = asset?.props?.src
  ...
  if (sourceUrl.startsWith(PAGE_ASSETS_ROUTE)) {
    return downloadSuperCowartFile({ assetUrl: sourceUrl, fileName })
  }
  ...
  return downloadSuperCowartFile({
    dataUrl,
    fileName,
...
async function exportSuperCowartHtmlDraft(editor, draftShapeId, format) {
  const shape = editor.getShape(draftShapeId)
  if (format === 'html') {
    const htmlContent = await readSuperCowartHtmlDraftContent(shape)
    const fileName = htmlDraftExportFileName(shape, 'html')
    const dataUrl = textDataUrl(htmlContent, 'text/html')
    if (hasSuperCowartWidgetBridge()) {
      return downloadSuperCowartFile({ dataUrl, fileName, mimeType: 'text/html' })
    }
  ...
  const exportResult = await renderSuperCowartHtmlDraftCanvas(
    shape,
...
async function exportSuperCowartSlides(editor, slidesShapeId, format) {
  if (!hasSuperCowartWidgetBridge()) {
    throw new Error('导出 Slides 文件夹需要在 Codex SuperCowart 小组件中使用。')
  }
  ...
  if (format === 'image') {
    for (const [index, item] of items.entries()) {
      let dataUrl
      if (isSuperCowartHtmlDraftEmbedShape(item)) {
        const bounds = new Box(0, 0, Number(item.props?.w) || 1, Number(item.props?.h) || 1)
        dataUrl = (await renderSuperCowartHtmlDraftCanvas(item, getAnnotationEditExportPixelRatio(bounds))).url
      } else {
      ...
      const result = await downloadSuperCowartFile({
        dataUrl,
        directoryName,
  ...
  for (const [index, item] of items.entries()) {
    ...
    let htmlContent
    if (isSuperCowartHtmlDraftEmbedShape(item)) {
      htmlContent = await readSuperCowartHtmlDraftContent(item)
    } else {
      const imageDataUrl = await readSuperCowartImageDataUrl(editor, item)
  ...
  results.push(await downloadSuperCowartFile({
...
async function exportSuperCowartHtmlDraftAnnotationScreenshot(editor, draftShapeId) {
  ...
  const draftCapture = await renderSuperCowartHtmlDraftCanvas(draftShape, pixelRatio)
...
async function exportSuperCowartSlidesAnnotationScreenshot(editor, slidesShapeId) {
  ...
  for (const item of getAiSlidesItems(editor, slidesShapeId)) {
    if (!isSuperCowartHtmlDraftEmbedShape(item)) continue
    const htmlCapture = await renderSuperCowartHtmlDraftCanvas(item, pixelRatio)
...
function buildAiSlidesAnnotationEditPrompt({
  ...
  const sourcePageLines = sourceItems.flatMap((item, index) => {
    if (isSuperCowartHtmlDraftEmbedShape(item)) {
      const assetPath = getSuperCowartHtmlDraftLocalPath(item)
  ...
  return [
    ...
    '',
    `SuperCowart source AI Slides frame: ${sourceSlidesShape.id}`,
    `SuperCowart target AI Slides frame below source: ${targetSlidesShapeId}`,
...
async function sendAiSlidesAnnotationEditRequest(editor, slidesShapeId) {
  ...
  const exportResult = await exportSuperCowartSlidesAnnotationScreenshot(editor, slidesShapeId)
  const screenshotAsset = await saveSuperCowartReferenceImage({
  ...
  try {
    const saveResult = await saveSuperCowartCanvasSnapshot(editor.store.getStoreSnapshot(), {
    ...
    await saveSuperCowartCanvasSnapshot(editor.store.getStoreSnapshot(), { protectImageRecords: true })
    throw error
  }
}
...
function buildHtmlDraftAnnotationEditPrompt({ draftShape, exportResult, screenshotAsset }) {
  const assetUrl = getSuperCowartHtmlDraftAssetUrl(draftShape)
  const assetPath = getSuperCowartHtmlDraftLocalPath(draftShape)
  const annotationCount = Math.max(0, exportResult.shapeIds.length - 1)
  ...
  return [
    ...
    '',
    `SuperCowart HTML draft shape: ${draftShape.id}`,
...
async function sendHtmlDraftAnnotationRequest(editor, draftShapeId, mode) {
  ...
  const draftShape = editor.getShape(draftShapeId)
  if (!isSuperCowartHtmlDraftEmbedShape(draftShape)) {
    throw new Error('请选择一个已生成 HTML 的 AI HTML。')
  }

  const exportResult = await exportSuperCowartHtmlDraftAnnotationScreenshot(editor, draftShapeId)
  const screenshotAsset = await saveSuperCowartReferenceImage({
...
function buildAiImageGenerationPrompt({ holderShape, userPrompt, references, referenceAttached }) {
  ...
  return [
    ...
    '',
    `SuperCowart AI image holder shape: ${holderShape.id}`,
...
function buildAiDraftGenerationPrompt({ holderShape, userPrompt, references, referenceAttached }) {
  ...
  return [
    ...
    '',
    `SuperCowart AI draft holder shape: ${holderShape.id}`,
...
function buildAiSlidesGenerationPrompt({ slidesShape, pageCount, userPrompt, references, referenceAttached }) {
  ...
  return [
    ...
    '',
    `SuperCowart AI Slides frame: ${slidesShape.id}`,
    `Required page count: exactly ${pageCount}.`,
...
async function persistGenerationTarget(editor, targetShapeId) {
  if (!editor?.getShape(targetShapeId)) {
    throw new Error(`SuperCowart 生成目标不存在：${targetShapeId}`)
  }
  const saveResult = await saveSuperCowartCanvasSnapshot(editor.store.getStoreSnapshot(), {
    protectImageRecords: true
  })
  if (saveResult?.ok === false) {
    throw new Error(saveResult.message || 'SuperCowart 生成目标保存失败。')
  }
}

async function sendAiImageGenerationRequest({ editor, holderShape, userPrompt, referenceFiles = [] }) {
  const sender = followUpSender()
  if (!sender) {
    throw new Error('当前 SuperCowart 画布没有可用的 Codex MCP 消息桥。')
  }

  await persistGenerationTarget(editor, holderShape.id)

  const imageReferences = referenceFiles.slice(0, AI_IMAGE_REFERENCE_MAX_FILES)
  const referenceAttached = Boolean(imageReferences.length && supportsSuperCowartMessageImages())
  const references = []
  ...
  for (const [index, referenceFile] of imageReferences.entries()) {
    ...
    if (hasSuperCowartWidgetBridge()) {
      try {
        savedReference = await saveSuperCowartReferenceImage({
        ...
        if (!referenceAttached) {
          throw new Error(`参考图无法保存到 SuperCowart 本地 assets：${error instanceof Error ? error.message : String(error)}`)
        }
        console.warn('SuperCowart reference image could not be saved; relying on direct image attachment.', error)
      }
    } else if (!referenceAttached) {
      throw new Error('当前 Codex host 没有声明支持图片附件，也没有可用的 SuperCowart MCP 文件保存桥。')
    }
...
async function sendAiDraftGenerationRequest({ editor, holderShape, userPrompt, referenceFiles = [] }) {
  const sender = followUpSender()
  if (!sender) {
    throw new Error('当前 SuperCowart 画布没有可用的 Codex MCP 消息桥。')
  }
  ...
  for (const [index, referenceFile] of imageReferences.entries()) {
    ...
    if (hasSuperCowartWidgetBridge()) {
      try {
        ...
        console.warn('SuperCowart draft reference image could not be saved; relying on direct image attachment.', error)
      }
    } else if (!referenceAttached) {
      throw new Error('当前 Codex host 没有声明支持图片附件，也没有可用的 SuperCowart MCP 文件保存桥。')
    }
...
async function sendAiSlidesGenerationRequest({ editor, slidesShape, pageCount, userPrompt, referenceFiles = [] }) {
  const sender = followUpSender()
  if (!sender) {
    throw new Error('当前 SuperCowart 画布没有可用的 Codex MCP 消息桥。')
  }

  await persistGenerationTarget(editor, slidesShape.id)
  ...
  for (const [index, referenceFile] of imageReferences.entries()) {
    ...
    if (hasSuperCowartWidgetBridge()) {
      try {
        ...
        console.warn('SuperCowart slides reference image could not be saved; relying on direct image attachment.', error)
      }
    } else if (!referenceAttached) {
      throw new Error('当前 Codex host 没有声明支持图片附件，也没有可用的 SuperCowart MCP 文件保存桥。')
    }
...
class SuperCowartAnnotationTool extends StateNode {
  static id = ANNOTATION_TOOL_ID
  static initial = 'idle'

  static children() {
    return [SuperCowartAnnotationIdle, SuperCowartAnnotationPointing]
  }
...
class SuperCowartAnnotationIdle extends StateNode {
  static id = 'idle'
...
class SuperCowartAnnotationPointing extends StateNode {
  static id = 'pointing'
...
class SuperCowartFrameShapeUtil extends FrameShapeUtil {
  isAspectRatioLocked(shape) {
    if (isSuperCowartAiHolderShape(shape)) {
...
const COWART_HTML_DRAFT_EMBED_DEFINITION = {
  ...
  embedOnPaste: false,
  toEmbedUrl(url) {
    if (isSuperCowartHtmlDraftDataUrl(url)) return url
...
function SuperCowartHtmlDraftEmbed({ shape }) {
  ...
  useEffect(() => {
    setHtmlSource(null)
    setLoadError(null)
    const shouldReadPageAsset = draftAssetUrl && hasSuperCowartWidgetBridge()
    ...
    const htmlSourcePromise = shouldReadPageAsset
      ? readSuperCowartPageAsset(draftAssetUrl).then((pageAsset) =>
    ...
    htmlSourcePromise
      .then((htmlContent) => hasSuperCowartWidgetBridge() ? hydrateSuperCowartHtmlDraftLocalImages(htmlContent) : htmlContent)
      .then((htmlContent) => {
      ...
      .catch((error) => {
        if (isDisposed) return
        console.warn('SuperCowart could not load HTML draft asset.', error)
  ...
  const persistDomEdits = useCallback(
    async (htmlContent) => {
      const result = await updateSuperCowartHtmlDraft({ draftShapeId: shape.id, htmlContent })
      const latestShape = editor.getShape(shape.id)
      if (!isSuperCowartHtmlDraftEmbedShape(latestShape)) return
      editor.updateShape({
        id: shape.id,
        type: 'embed',
        meta: {
  ...
  useEffect(() => {
    ...
    waitForHtmlDraftDocument(shape.id)
      .then((iframeDocument) => {
        if (disposed) return
        session = createSuperCowartHtmlDraftDomEditorSession(iframeDocument, {
          onSave: persistDomEdits,
          onRequestExit: exitDomEditing
        })
        cowartHtmlDraftDomEditSessions.set(shape.id, session)
      })
      .catch((error) => console.error('SuperCowart could not start HTML DOM editing.', error))
    ...
    return () => {
      ...
      session
        .dispose({ save: true })
        .catch((error) => console.error('SuperCowart could not save HTML DOM edits.', error))
    }
...
const SuperCowartConfiguredEmbedShapeUtil = EmbedShapeUtil.configure({
  embedDefinitions: [COWART_HTML_DRAFT_EMBED_DEFINITION, ...DEFAULT_EMBED_DEFINITIONS]
})

class SuperCowartEmbedShapeUtil extends SuperCowartConfiguredEmbedShapeUtil {
  component(shape) {
    if (isSuperCowartHtmlDraftEmbedShape(shape)) {
      return <SuperCowartHtmlDraftEmbed shape={shape} />
    }
...
const cowartShapeUtils = [SuperCowartFrameShapeUtil, SuperCowartEmbedShapeUtil]
        ...
        async onSelect(source) {
          ...
          try {
            ...
            for (const imageShape of imageShapes) {
              results.push(await downloadSuperCowartImageShape(editor, imageShape))
            }
            helpers.addToast({
...
const cowartComponents = {
  Toolbar: SuperCowartToolbar,
  ImageToolbar: SuperCowartSelectionToolbar,
  InFrontOfTheCanvas: SuperCowartCanvasOverlay,
  StylePanel: SuperCowartStylePanel
}

function SuperCowartCanvasOverlay() {
  return (
    <>
      <SuperCowartAiImageGenerationPanel />
      <SuperCowartAiDraftGenerationPanel />
      <SuperCowartAiSlidesGenerationPanel />
      <SuperCowartSlidesPresentationOverlay />
    </>
  )
}
...
function SuperCowartSlidesMedia({ onUnhandledHtmlClick, shape, title }) {
  const editor = useEditor()
  ...
  useEffect(() => {
    ...
    async function loadSource() {
      if (isImageShape(shape)) {
        ...
        if (assetSource.startsWith(PAGE_ASSETS_ROUTE) && hasSuperCowartWidgetBridge()) {
          const pageAsset = await readSuperCowartPageAsset(assetSource)
          return {
            kind: 'image',
      ...
      if (isSuperCowartHtmlDraftEmbedShape(shape)) {
        const directHtmlUrl = isSuperCowartHtmlDraftDataUrl(shape.props.url) ? shape.props.url : null
        if (directHtmlUrl) {
          if (!hasSuperCowartWidgetBridge()) return { kind: 'html-url', url: directHtmlUrl }
          const response = await window.fetch(directHtmlUrl)
          if (!response.ok) throw new Error(`HTML 草稿加载失败：${response.status}`)
          return {
            kind: 'html',
            htmlContent: await hydrateSuperCowartHtmlDraftLocalImages(await response.text())
          }
        }
        ...
        if (hasSuperCowartWidgetBridge()) {
          ...
          return {
            kind: 'html',
            htmlContent: await hydrateSuperCowartHtmlDraftLocalImages(htmlContent)
          }
        }
...
function SuperCowartSlidesScaledMedia({ className = '', onUnhandledHtmlClick, shape, title }) {
...
function SuperCowartSlidesPresentationOverlay() {
  const editor = useEditor()
...
function SuperCowartAiImageGenerationPanel() {
  const editor = useEditor()
...
function SuperCowartAiDraftGenerationPanel() {
  const editor = useEditor()
  const selectedTarget = useValue(
...
function SuperCowartAiSlidesGenerationPanel() {
  const editor = useEditor()
  const selectedTarget = useValue(
...
function SuperCowartStylePanel(props) {
  return (
    <DefaultStylePanel {...props}>
      <DefaultStylePanelContent />
      <SuperCowartAiImageStyleControls />
    </DefaultStylePanel>
  )
}

function SuperCowartAiImageStyleControls() {
  const editor = useEditor()
  const selectedAiHolderShape = useValue(
    ...
    () => {
      ...
      return isSuperCowartAiHolderShape(shape) ? shape : null
    },
    [editor]
  )
      ...
      <section className="cowart-ai-style-section">
        ...
        <div className="cowart-ai-size-row">
          ...
          <button
            ...
            type="button"
          >
            <SuperCowartAspectLockIcon locked={isAspectLocked} />
          </button>
          <label className="cowart-ai-size-field">
            <span>H</span>
            <input
...
function SuperCowartAspectLockIcon({ locked }) {
  if (locked) {
    return (
      <svg
        aria-hidden="true"
...
function SuperCowartSelectionToolbar() {
  const editor = useEditor()
  ...
  const htmlDraftShapeId = useValue(
    ...
    () => {
      const shape = editor.getOnlySelectedShape()
      return isSuperCowartHtmlDraftEmbedShape(shape) ? shape.id : null
    },
    [editor]
  )

  if (slidesShapeId) {
    return <SuperCowartSlidesToolbar slidesShapeId={slidesShapeId} />
  }

  if (htmlDraftShapeId) {
    return <SuperCowartHtmlDraftToolbar draftShapeId={htmlDraftShapeId} />
  }

  return <SuperCowartImageToolbar />
}

function SuperCowartExportMenu({ disabled = false, onExportHtml, onExportImage, targetKey }) {
  const editor = useEditor()
...
function SuperCowartSlidesToolbar({ slidesShapeId }) {
  const editor = useEditor()
  ...
  return (
    <TldrawUiContextualToolbar
      ...
      label="AI Slides 工具栏"
    >
      <SuperCowartExportMenu
        disabled={!hasContent}
        onExportHtml={() => exportSuperCowartSlides(editor, slidesShapeId, 'html')}
        onExportImage={() => exportSuperCowartSlides(editor, slidesShapeId, 'image')}
      ...
      {hasContent && <SuperCowartSlidesAnnotationEditButton slidesShapeId={slidesShapeId} />}
    </TldrawUiContextualToolbar>
  )
}

function SuperCowartSlidesAnnotationEditButton({ slidesShapeId }) {
  const editor = useEditor()
  const [status, setStatus] = useState('idle')
...
function SuperCowartHtmlDraftToolbar({ draftShapeId }) {
  const editor = useEditor()
  const showToolbar = useValue(
  ...
  return (
    ...
    >
      {!isDomEditing && (
        <SuperCowartExportMenu
          onExportHtml={() => exportSuperCowartHtmlDraft(editor, draftShapeId, 'html')}
          onExportImage={() => exportSuperCowartHtmlDraft(editor, draftShapeId, 'image')}
          targetKey={`html-${draftShapeId}`}
        />
      )}
      <SuperCowartHtmlDraftDomEditButton draftShapeId={draftShapeId} isEditing={isDomEditing} />
      {!isDomEditing && (
        <>
          <SuperCowartHtmlDraftToolbarButton
            action="edit"
...
function SuperCowartHtmlDraftDomEditButton({ draftShapeId, isEditing }) {
  const editor = useEditor()
  const [status, setStatus] = useState('idle')
...
function SuperCowartHtmlDraftToolbarButton({
  action,
  draftShapeId,
  icon,
  iconElement,
...
function SuperCowartImageToolbar() {
  return (
    <DefaultImageToolbar>
      <SuperCowartImageToolbarContent />
    </DefaultImageToolbar>
  )
}

function SuperCowartImageToolbarContent() {
  const editor = useEditor()
  ...
  return (
    <>
      ...
      {!isInCropTool && (
        <>
          <SuperCowartAnnotationEditToolbarButton imageShapeId={imageShapeId} />
          <SuperCowartAnnotationHtmlToolbarButton imageShapeId={imageShapeId} />
        </>
      )}
    </>
  )
...
function SuperCowartAltTextEditor({ shapeId, onClose }) {
  const editor = useEditor()
...
function SuperCowartAnnotationEditToolbarButton({ imageShapeId }) {
  const editor = useEditor()
  const [status, setStatus] = useState('idle')
...
function SuperCowartAnnotationHtmlToolbarButton({ imageShapeId }) {
  const editor = useEditor()
  const [status, setStatus] = useState('idle')
...
function SuperCowartToolbarItem({ toolId }) {
  const editor = useEditor()
...
function SuperCowartAnnotationToolbarItem() {
  const editor = useEditor()
  const isSelected = useValue(
...
function SuperCowartToolbarDivider() {
  return <div aria-orientation="vertical" className="cowart-toolbar-divider" role="separator" />
}

function SuperCowartToolbar(props) {
  return (
    <DefaultToolbar {...props} maxItems={11}>
      <SuperCowartAnnotationToolbarItem />
      <SuperCowartToolbarDivider />
      <SelectToolbarItem />
      <HandToolbarItem />
      <SuperCowartToolbarItem toolId={AI_IMAGE_TOOL_ID} />
      <SuperCowartToolbarItem toolId={AI_DRAFT_TOOL_ID} />
      <SuperCowartToolbarItem toolId={AI_SLIDES_TOOL_ID} />
      <SuperCowartToolbarDivider />
      <AssetToolbarItem />
      <DrawToolbarItem />
      <EraserToolbarItem />
...
function getSuperCowartSelection(editor) {
  const selectedShapeIds = editor.getSelectedShapeIds()
  return selectedShapeIds.map((id) => {
    ...
    return {
      ...
      isHtmlDraft: isSuperCowartHtmlDraftEmbedShape(shape),
...
function getSuperCowartSelectionSnapshot(editor) {
  return {
    selectedShapes: getSuperCowartSelection(editor)
  }
}

function getSuperCowartViewState(editor) {
...
function restoreSuperCowartViewState(editor, viewState) {
...
function writeSuperCowartSelectionState(selectionSnapshot) {
...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function loadCanvas() {
      try {
        const canvasState = await loadSuperCowartCanvasState(controller.signal)
  ...
  const handleMount = useCallback((editor) => {
    ...
    window.__cowartSelection = () => getSuperCowartSelection(editor)
    window.__cowartViewState = () => getSuperCowartViewState(editor)
    ...
    editor.timers.requestAnimationFrame(() => {
      restoreSuperCowartViewState(editor, viewState)
    })

    async function syncSelectionState() {
      const selectionSnapshot = getSuperCowartSelectionSnapshot(editor)
      writeSuperCowartSelectionState(selectionSnapshot)
      ...
      try {
        await saveSuperCowartSelectionState({
          ...selectionSnapshot,
          updatedAt: new Date().toISOString()
        })
      } catch (error) {
    ...
    async function syncViewState() {
      const viewStateSnapshot = {
        ...getSuperCowartViewState(editor),
        updatedAt: new Date().toISOString()
      }
      ...
      try {
        await saveSuperCowartViewState(viewStateSnapshot)
      } catch (error) {
        console.error(error)
      } finally {
    ...
    function handleSuperCowartCopy(event) {
      copySelectedSuperCowartContent(editor, event)
    }
    containerDocument.addEventListener('copy', handleSuperCowartCopy, { capture: true })
    ...
    async function saveCanvas() {
      ...
      try {
        ...
        if (saveResult?.ok === false) {
          throw new Error(saveResult.message || 'SuperCowart refused to save the canvas snapshot.')
        }
    ...
    async function loadRemoteCanvasSnapshot() {
      ...
      try {
        const nextSnapshot = await refreshSuperCowartCanvasSnapshot(controller.signal)
    ...
    if (hasSuperCowartWidgetBridge()) {
      ...
      canvasEvents.onerror = (error) => {
        console.warn('SuperCowart canvas live refresh disconnected.', error)
      }
    }
    ...
    return () => {
      ...
      containerDocument.removeEventListener('copy', handleSuperCowartCopy, { capture: true })
  ...
  return (
    <main className="supercowart-canvas" aria-label="SuperCowart infinite canvas">
      ...
      <Tldraw
        ...
        tools={[SuperCowartAnnotationTool]}
      />
    </main>
  )
}
