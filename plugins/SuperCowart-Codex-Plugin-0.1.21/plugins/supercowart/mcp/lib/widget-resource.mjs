...
function mcpHostBridgeScript() {
  ...
  function toBridgeError(error) {
    ...
    return new Error(String(error || "SuperCowart host bridge is unavailable."));
  }
  ...
  async function waitForReady(app) {
    if (app?.ready) {
      await withTimeout(app.ready, 4000, "SuperCowart host bridge did not become ready.");
    }
  ...
  function installSuperCowartApi(app) {
    ...
    api.callServerTool = async (request, options) => {
      try {
        ...
        return await withTimeout(
          ...
          "SuperCowart server tool call timed out.",
        );
      } catch (error) {
        throw toBridgeError(error);
      }
