...
function canvasStoragePlugin() {
  return {
    name: 'supercowart-canvas-storage',
    configureServer(server) {
      ...
      server.middlewares.use('/api/html-draft', async (req, res) => {
        try {
          ...
          if (!loaded.snapshot) {
            sendJson(res, 404, { error: 'No SuperCowart canvas snapshot exists.' })
            return
          }
        ...
        try {
          ...
          if (req.method === 'PUT') {
            ...
            if (!isSelectionState(selection)) {
              sendJson(res, 400, { error: 'Expected a SuperCowart selection state.' })
              return
            }
        ...
        try {
          ...
          if (req.method === 'PUT') {
            ...
            if (!isViewState(viewState)) {
              sendJson(res, 400, { error: 'Expected a SuperCowart view state.' })
              return
            }
