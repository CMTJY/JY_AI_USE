...
export function hasSuperCowartWidgetBridge() {
...
async function waitForWidgetPayload(signal) {
  if (!hasSuperCowartWidgetBridge()) return
  ...
  await new Promise((resolve, reject) => {
    ...
    const timer = window.setTimeout(() => {
      cleanup()
      reject(new Error('SuperCowart widget storage target was not ready. Refusing to read or write without projectDir/canvasDir.'))
...
async function callSuperCowartServerTool(name, args = {}, options = {}) {
  ...
  if (result?.isError) {
    ...
    throw new Error(message || `SuperCowart server tool failed: ${name}`)
  }
...
async function fetchJson(url, options = {}) {
  ...
  if (!response.ok) {
    throw new Error(`SuperCowart request failed: ${response.status} - ${response.statusText}`)
  }
  return response.json()
}

export async function loadSuperCowartCanvasState(signal) {
  if (hasSuperCowartWidgetBridge()) {
    const state = await callSuperCowartServerTool(
...
export async function refreshSuperCowartCanvasSnapshot(signal) {
  if (hasSuperCowartWidgetBridge()) {
    const state = await callSuperCowartServerTool(
      TOOL_GET_CANVAS_STATE,
      { hydrateAssets: false },
...
export async function saveSuperCowartCanvasSnapshot(snapshot, options = {}) {
  if (hasSuperCowartWidgetBridge()) {
    return callSuperCowartServerTool(TOOL_SAVE_CANVAS_STATE, {
      snapshot,
...
export async function saveSuperCowartSelectionState(selection) {
  if (hasSuperCowartWidgetBridge()) {
    return callSuperCowartServerTool(TOOL_SAVE_SELECTION_STATE, { selection })
  }
...
export async function saveSuperCowartViewState(viewState) {
  if (hasSuperCowartWidgetBridge()) {
    return callSuperCowartServerTool(TOOL_SAVE_VIEW_STATE, { viewState })
  }
...
export async function saveSuperCowartReferenceImage(reference) {
  if (!hasSuperCowartWidgetBridge()) {
    throw new Error('当前 SuperCowart 画布没有可用的 Codex MCP 文件保存桥。')
  }

  return callSuperCowartServerTool(TOOL_SAVE_REFERENCE_IMAGE, reference)
}

export async function downloadSuperCowartFile(download) {
  if (!hasSuperCowartWidgetBridge()) {
    throw new Error('当前 SuperCowart 画布没有可用的 Codex MCP 文件下载桥。')
  }

  return callSuperCowartServerTool(TOOL_DOWNLOAD_FILE, download)
}

export async function updateSuperCowartHtmlDraft({ draftShapeId, htmlContent }) {
  if (!hasSuperCowartWidgetBridge()) {
  ...
  return callSuperCowartServerTool(TOOL_INSERT_HTML_DRAFT, {
    draftShapeId,
    htmlContent,
...
export async function readSuperCowartPageAsset(assetUrl, options = {}) {
  if (!hasSuperCowartWidgetBridge()) {
    throw new Error('当前 SuperCowart 画布没有可用的 Codex MCP 文件读取桥。')
  }

  return callSuperCowartServerTool(TOOL_READ_PAGE_ASSET, { assetUrl }, options)
}
