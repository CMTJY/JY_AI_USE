---
name: supercowart-image-edit
description: Generate new AI images from user-supplied SuperCowart annotation screenshots. Use when the user provides one or more screenshots showing SuperCowart images marked with the 批注 tool, arrows, or visible edit notes and wants Codex to apply those requested changes, create revised bitmap images, and place each result beside the corresponding original or in a nearby clear area without replacing, moving, hiding, or deleting the original images or annotations.
---

# SuperCowart Image Edit

Use this skill to turn user-provided SuperCowart 批注 screenshots into revised AI-generated bitmaps placed next to the corresponding original images.

## Preconditions

The native SuperCowart widget should be open for the active project. SuperCowart state is read and written through SuperCowart MCP tools, not through a localhost browser service.
...
3. Choose the source image for generation.
   ...
   Do not read the current SuperCowart canvas to discover edit intent. Use the screenshot for the requested changes. SuperCowart state may be read later only to place the generated result without covering existing content.
...
5. Generate a new bitmap.
   ...
   Before inserting the resolved file into SuperCowart, visually inspect the local bitmap and confirm it is the newly generated revised image for this screenshot, not a stale generated asset.

6. Insert the revised image beside the original with SuperCowart MCP.

   Prefer the SuperCowart MCP `insert_cowart_image` tool. Do not hand-write
   ...
   overlaps, and saves through the project-backed SuperCowart canvas files.
   ...
   - If the screenshot clearly shows the original image and there is a unique matching generated/original image or `AI 图片` frame on the current SuperCowart page, use that as the anchor without asking the user to select it.
...
7. Save through SuperCowart.

   Only do SuperCowart state access after the bitmap is generated. Use this access only to insert the new image beside the anchor or in a nearby clear area, not to discover edit intent.
   ...
   ```json
   {
     ...
     "altText": "Revised image generated from SuperCowart annotation screenshot"
   }
   ```
   ...
   SuperCowart's MCP storage layer will preserve per-page snapshots under:
...
8. Verify visually.
   ...
   Let the SuperCowart widget refresh from MCP-backed storage, then confirm:
