...
export function resolveSuperCowartPaths(args = {}) {
...
export function resolveCanvasDir(args = {}) {
  return resolveSuperCowartPaths(args).canvasDir;
}
...
export async function writeSuperCowartPageAsset(args = {}, options = {}) {
  const pageId = nonEmptyString(options.pageId);
  if (!pageId) throw new Error("pageId is required to save a SuperCowart page asset.");
  ...
  if (!mimeType.startsWith("image/")) {
    throw new Error(`SuperCowart page assets only accept image payloads. Received ${mimeType}.`);
  }

  const canvasDir = resolveCanvasDir(args);
  const destinationDir = pageAssetsDir(args, pageId);
  if (!isSafeChildPath(canvasDir, destinationDir)) {
    throw new Error(`Unsafe SuperCowart page assets directory: ${destinationDir}`);
  }
...
export async function readSuperCowartPageAsset(args = {}, options = {}) {
  const assetUrl = nonEmptyString(options.assetUrl);
  if (!assetUrl) throw new Error("assetUrl is required to read a SuperCowart page asset.");
  if (!assetUrl.startsWith(PAGE_ASSETS_ROUTE) && !assetUrl.startsWith(GLOBAL_ASSETS_ROUTE)) {
    throw new Error(`Unsupported SuperCowart asset URL: ${assetUrl}`);
  }

  const filePath = localAssetFilePathFromUrl(assetUrl, args);
  if (!filePath) throw new Error(`Unsafe SuperCowart asset URL: ${assetUrl}`);

  const fileStat = await stat(filePath);
  if (!fileStat.isFile()) throw new Error(`SuperCowart asset is not a file: ${assetUrl}`);
  ...
  if (!mimeType.startsWith("image/") && mimeType !== "text/html") {
    throw new Error(`SuperCowart page assets only expose image or HTML payloads. Received ${mimeType}.`);
  }
...
export async function readSuperCowartCanvasState(args = {}, { hydrateAssets = false } = {}) {
  const { projectDir, canvasDir } = resolveSuperCowartPaths(args);
  ...
  const { viewState, viewStateFile } = await readSuperCowartViewState(args);
...
export async function saveSuperCowartCanvasSnapshot(args = {}, snapshot) {
  ...
  if (imageLosses.length > 0) {
    return {
      ...
      message: `SuperCowart refused to save because ${imageLosses.length} existing image shape(s) disappeared without a user delete confirmation.`,
    };
  }
...
export async function readSuperCowartSelectionState(args = {}) {
...
export async function writeSuperCowartSelectionState(args = {}, selection) {
  if (!isSelectionState(selection)) {
    throw new Error("Expected a SuperCowart selection state.");
  }
  const selectionFile = resolveSelectionFile(args);
  const payload = {
    ...selection,
...
export async function readSuperCowartViewState(args = {}) {
...
export async function writeSuperCowartViewState(args = {}, viewState) {
  if (!isViewState(viewState)) {
    throw new Error("Expected a SuperCowart view state.");
  }
  const viewStateFile = resolveViewStateFile(args);
  const payload = {
    ...viewState,
