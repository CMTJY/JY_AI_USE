...
export async function cowartStaticHtml() {
  ...
  pendingStaticHtml ??= buildSuperCowartStaticHtml().finally(() => {
...
async function buildSuperCowartStaticHtml() {
...
async function ensureViteBinary() {
  ...
  if (!existsSync(viteBinaryPath())) {
    throw new Error("Missing Vite dependency after npm install in the SuperCowart plugin directory.");
  }
}
...
function runNpmInstall() {
  ...
  return runCommand(command, args, {
    cwd: pluginRoot(),
    failureLabel: "npm install failed while preparing the SuperCowart widget",
  });
}
...
function runViteBuild(outDir) {
  return runCommand(process.execPath, [
    ...
    failureLabel: "Vite build failed while preparing the SuperCowart widget",
  });
}
...
async function inlineViteBuild(outDir) {
  ...
  if (/\b(?:src|href)\s*=\s*"[^"]*\/assets\//i.test(html)) {
    throw new Error("The SuperCowart widget still references external build assets.");
  }
  ...
  if (existsSync(assetsDir)) {
    ...
    if (leftovers.length > 0) {
      throw new Error(
        `The SuperCowart widget build emitted non-inlined assets: ${leftovers.join(", ")}`,
      );
    }
  }
...
function assertCspCompatibleStaticHtml(html) {
  ...
  for (const [pattern, label] of forbiddenShellPatterns) {
    if (pattern.test(shellMarkup)) {
      throw new Error(`The SuperCowart widget is not CSP-compatible: found ${label}.`);
    }
  }
  ...
  for (const value of resourceAttributeValues(shellMarkup)) {
    if (isExternalResourceValue(value)) {
      throw new Error(
        `The SuperCowart widget is not CSP-compatible: found external resource ${value}.`,
      );
    }
  }
  ...
  if (/\bfetch\s*\(/i.test(html) && !html.includes("__COWART_WIDGET_FETCH_GUARD__")) {
    throw new Error(
      "The SuperCowart widget is not CSP-compatible: found fetch() without the fetch guard.",
    );
  }
}
